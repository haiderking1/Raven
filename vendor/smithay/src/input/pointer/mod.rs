...
pub use motion_hook::{MotionHook, MotionPhase};
...
pub(crate) struct PointerInternal<D: SeatHandler> {
    ...
    motion_hook: Option<MotionHook<D>>,
}
...
impl<D: SeatHandler + 'static> PointerInternal<D> {
    ...
    fn motion(
        ...
        if let Some(hook) = self.motion_hook {
            if !hook(data, seat, MotionPhase::Before, &old_focus, &focus, event) {
                return;
            }
        }
        ...
        if let Some(hook) = self.motion_hook {
            hook(data, seat, MotionPhase::After, &old_focus, &self.focus, event);
        }
    }
