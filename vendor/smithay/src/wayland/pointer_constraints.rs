...
pub trait PointerConstraintsHandler: SeatHandler {
    ...
    /// A region or hint was committed. Called without the constraint mutex.
    fn constraint_committed(&mut self, _surface: &WlSurface, _pointer: &PointerHandle<Self>) {}

    /// A protocol resource was removed. Called without the constraint mutex.
    fn constraint_destroyed(
        &mut self,
        _surface: &WlSurface,
        _pointer: &PointerHandle<Self>,
        _constraint: PointerConstraint,
    ) {
    }
...
impl<D: SeatHandler + 'static> PointerConstraintRef<'_, D> {
    ...
    ///
    /// The compositor must call this on focus loss and policy-driven release.
...
fn commit_hook<D: SeatHandler + PointerConstraintsHandler + 'static>(
    ...
    let position_hints = committed::apply::<D>(surface);

    for (pointer, hint) in position_hints {
        if let Some(hint) = hint {
            state.cursor_position_hint(surface, &pointer, hint);
        }
        state.constraint_committed(surface, &pointer);
    }
}
...
fn add_constraint<D: SeatHandler + PointerConstraintsHandler + 'static>(
    ...
    if added {
        compositor::add_pre_commit_hook(surface, committed::snapshot::<D>);
        compositor::add_post_commit_hook(surface, commit_hook::<D>);
    }
}

fn remove_constraint<D: PointerConstraintsHandler + 'static>(
    state: &mut D,
    surface: &WlSurface,
    pointer: &PointerHandle<D>,
    resource: wayland_server::backend::ObjectId,
) {
    let removed = with_constraint_data::<D, _, _>(surface, |data| {
        let data = data?;
        let constraint = data.constraints.get(pointer)?;
        let id = match constraint {
            PointerConstraint::Locked(locked) => locked.handle.id(),
            PointerConstraint::Confined(confined) => confined.handle.id(),
        };
        // A retired oneshot resource must not remove a newer constraint.
        (id == resource)
            .then(|| data.constraints.remove(pointer))
            .flatten()
    });
    if let Some(constraint) = removed {
        state.constraint_destroyed(surface, pointer, constraint);
    }
}
    ...
    D: 'static,
{
        ...
        match request {
            zwp_confined_pointer_v1::Request::SetRegion { region } => {
                with_pointer_constraint(&data.surface, pointer, |constraint| {
                    ...
                    {
                        if &confined.handle != confined_pointer {
                            return;
                        }
    ...
    fn destroyed(
        ...
        remove_constraint(state, &data.surface, pointer, resource.id());
    }
}
            ...
            zwp_locked_pointer_v1::Request::SetCursorPositionHint { surface_x, surface_y } => {
                with_pointer_constraint(&data.surface, pointer, |constraint| {
                    if let Some(PointerConstraint::Locked(locked)) = constraint.map(|x| x.entry.into_mut()) {
                        if &locked.handle != locked_pointer {
                            return;
                        }
