use super::motion::send as motion;
...
fn pointer_movement_requests_redraw_but_stationary_input_does_not() {
    ...
    motion(&mut state, (10.0, 20.0).into(), 1, None, true);
    assert!(state.take_redraw_request());
    motion(&mut state, (10.0, 20.0).into(), 2, None, true);
    assert!(!state.take_redraw_request());
    motion(&mut state, (11.0, 20.0).into(), 3, None, true);
    assert!(state.take_redraw_request());
}
