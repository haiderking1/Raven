...
fn workspace_wire_transactions_and_lifecycle() {
    ...
    f.state.workspace_protocol.persistent.fill(false);
    let hidden = smithay::reexports::wayland_protocols::ext::workspace::v1::server::ext_workspace_handle_v1::State::Hidden.bits();
    ...
    assert_eq!(first.states(&events), [(0, hidden), (2, 1)]);
    assert_eq!(second.states(&events), [(0, hidden), (2, 1)]);
    first.done(&events);
    second.done(&events);
    ...
    let events = dispatch(&mut f);
    assert_eq!(f.state.workspaces.active, 2);
    assert_eq!(first.states(&events), [(3, hidden)]);
    ...
    f.state.refresh_workspace_protocol();
    let events = dispatch(&mut f);
    assert_eq!(first.states(&events), [(2, hidden), (9, 1)]);
    assert_eq!(second.states(&events), [(2, hidden), (9, 1)]);
    first.done(&events);
    second.done(&events);
    f.state.refresh_workspace_protocol();
    assert!(dispatch(&mut f).is_empty());
    ...
    let events = dispatch(&mut f);
    assert!(!events.iter().any(|e| first.owns(e)));
    assert_eq!(second.states(&events), [(0, 1), (9, hidden)]);
    second.done(&events);
